...
auto OrderBook::cancel(ClientId client_id, OrderId order_id, TickerId ticker_id) noexcept
  -> void {
  ...
  if (UNLIKELY(!is_cancelable)) {
    ...
    market_update_ = {
      MarketUpdateType::CANCEL, exchange_order->market_order_id_, ticker_id,
...
auto OrderBook::toString(bool detailed, bool validity_check) const -> std::string {
  ...
  auto printer = [&](
    ...
    for (auto o_itr = itr->first_me_order_;; o_itr = o_itr->next_order_) {
      if (detailed) {
        sprintf(
          ...
          orderIdToString(
            o_itr->prev_order_ ? o_itr->prev_order_->market_order_id_ : OrderId_INVALID)
            .c_str(),
          orderIdToString(
            o_itr->next_order_ ? o_itr->next_order_->market_order_id_ : OrderId_INVALID)
            .c_str());
        ss << buf;
      }
      if (o_itr->next_order_ == itr->first_me_order_) {
    ...
    if (sanity_check) {
      if (
        (side == Side::SELL && last_price >= itr->price_)
